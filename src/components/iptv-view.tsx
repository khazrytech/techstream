...
import { Search, Heart, Play, Lock, UserPlus, LogIn, CheckCircle } from "lucide-react";
...
export function IPTVView({ defaultCategory = "All" }: IPTVViewProps) {
  ...
  // Orodha ya Kategoria zote zilizopo
  ...
  const filteredChannels = allChannels.filter((ch) => {
    ...
    const matchesCat =
      activeCategory === "All"
        ? true
        : activeCategory.toLowerCase() === "movies"
        ? /movie|cinema|film|vod|action|hbo|box/i.test(ch.name + " " + ch.group)
        : activeCategory.toLowerCase() === "series"
        ? /series|serial|drama|show|season/i.test(ch.name + " " + ch.group)
        : activeCategory.toLowerCase() === "live tv"
        ? true
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-black text-zinc-400 flex items-center justify-center text-sm font-semibold">
        Inapakia chaneli za TechStream...
      </div>
    );
  }

  const liveHeaderChannels = allChannels.slice(0, 6);
    ...
    <div className="min-h-screen bg-black text-white p-4 pb-28 space-y-6">
      ...
      <div className="space-y-3">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-xs font-black tracking-wider text-red-500 uppercase">LIVE SASA</span>
        </div>
        ...
        <div className="flex space-x-3 overflow-x-auto no-scrollbar pb-2">
          {liveHeaderChannels.map((ch) => (
            <div
              ...
              className="min-w-[180px] bg-zinc-900/80 border border-zinc-800 rounded-2xl p-3 relative flex flex-col justify-between cursor-pointer hover:border-zinc-700 transition-all"
            >
              <span className="absolute top-2 right-2 bg-red-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded">
                LIVE
              </span>
              <div className="w-10 h-10 bg-zinc-950 rounded-xl p-1 mb-3 flex items-center justify-center border border-zinc-800">
                {ch.logo ? (
              ...
              <div>
                <h4 className="text-sm font-bold text-white line-clamp-1">{ch.name}</h4>
                <p className="text-[11px] text-zinc-500">{ch.group}</p>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="w-full aspect-video bg-black rounded-xl overflow-hidden flex items-center justify-center relative border border-zinc-800">
            {isLoggedIn ? (
              ...
              <div className="absolute inset-0 bg-zinc-950/95 backdrop-blur-md flex flex-col items-center justify-center p-6 text-center space-y-3">
                ...
                <h3 className="text-lg font-bold text-white">Ingia au Jisajili Kutazama</h3>
                <button
      ...
      <div className="flex space-x-2 overflow-x-auto no-scrollbar py-1">
        {categoryNames.slice(0, 10).map((cat) => (
          <button
            key={cat}
      ...
      {/* CHANNELS GRID (2 COLUMNS matching original UI) */}
          ...
          return (
            <div
              ...
              className={`bg-zinc-900/90 border rounded-2xl p-3 flex items-center justify-between cursor-pointer transition-all ${
                isSelected ? "border-red-600 ring-1 ring-red-600/50" : "border-zinc-800/80 hover:border-zinc-700"
              }`}
            >
              <div className="flex items-center space-x-3 overflow-hidden">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-[10px] text-zinc-500 line-clamp-1">{ch.group}</p>
                </div>
              </div>

...
export default function MoviesPage() {
    ...
    fetch("/api/iptv")
      ...
      .then((data) => {
        if (data.success) {
          ...
          const movieFilter = all.filter((c) =>
            /movie|cinema|film|vod|action|hbo|box/i.test(c.name + " " + c.group)
          );
          setChannels(movieFilter.length > 0 ? movieFilter : all);
        }
        setLoading(false);
      });
  }, []);
  ...
  return (
    <div className="p-4 pb-24 text-white">
      ...
      {loading ? (
        <p className="text-zinc-500">Inapakia Movies...</p>
      ) : (
        <>
